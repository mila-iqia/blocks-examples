...
def get_tr_stream(config):
    ...
    src_vocab = _ensure_special_tokens(cPickle.load(open(src_vocab_file)),
                                       bos_idx=0,
                                       eos_idx=config['src_vocab_size'] - 1,
                                       unk_idx=config['unk_id'])
    trg_vocab = _ensure_special_tokens(cPickle.load(open(trg_vocab_file)),
                                       bos_idx=0,
    ...
    stream = Batch(
        stream, iteration_scheme=ConstantScheme(config['batch_size']))

    # Pad sequences that are short
    masked_stream = PaddingWithEOS(
        stream, [config['src_vocab_size'] - 1, config['trg_vocab_size'] - 1])
...
def get_dev_stream(config):
    ...
    if 'val_set' in config and config['val_set']:
        ...
        src_vocab = _ensure_special_tokens(
            cPickle.load(open(config['src_vocab'])), bos_idx=0,
            eos_idx=config['src_vocab_size'] - 1, unk_idx=config['unk_id'])
